...
export function AffiliateDashboard() {
  ...
  const cards = [
    ...
    { label: "Comissões Pagas", value: `R$ ${(stats?.paid_commissions || 0).toFixed(2)}`, icon: CheckCircle, color: "bg-emerald-500" },
  ];
    ...
    <div className="min-h-screen bg-zinc-50 font-sans text-zinc-900">
      <nav className="bg-white border-b border-zinc-200 sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
          <h1 className="text-xl font-bold text-emerald-600">Painel de Afiliado</h1>
          <div className="flex items-center gap-4">
            <span className="text-sm font-medium text-zinc-500 hidden md:block">Olá, {user.name}</span>
            <button 
      ...
      <main className="max-w-7xl mx-auto px-4 py-8 space-y-8">
        ...
        <section className="bg-emerald-600 text-white p-8 rounded-3xl shadow-lg shadow-emerald-100 space-y-4">
          <div className="space-y-1">
            <h2 className="text-2xl font-bold">Seu Link de Afiliado</h2>
            <p className="text-emerald-100">Compartilhe este link para ganhar comissões em cada venda.</p>
          </div>
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <button 
              onClick={copyLink}
              className="bg-white text-emerald-600 px-8 py-3 rounded-xl font-bold hover:bg-emerald-50 transition-colors flex items-center justify-center gap-2"
            >
                        ...
                        <tr key={c.id} className="hover:bg-zinc-50 transition-colors">
                          ...
                          <td className="px-6 py-4">
                            <span className={cn(
                              ...
                              c.status === 'pending' ? "bg-yellow-100 text-yellow-600" : "bg-emerald-100 text-emerald-600"
                            )}>
                    ...
                    <div key={c.id} className="p-4 space-y-3">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="text-right">
                          ...
                          <p className="font-bold text-emerald-600">R$ {c.amount.toFixed(2)}</p>
                        </div>
                      </div>
                    </div>
                  ))
          ...
          <section className="space-y-6">
            <div className="bg-zinc-900 text-white p-8 rounded-3xl shadow-xl space-y-6">
              <h2 className="text-xl font-bold flex items-center gap-2">
                <MessageCircle size={24} className="text-emerald-400" /> Dicas de Vendas
              </h2>
              <div className="space-y-4">
                <div className="p-4 bg-white/5 rounded-2xl border border-white/10 space-y-2">
                  <h4 className="font-bold text-sm text-emerald-400">WhatsApp é Poder</h4>
                ...
                <div className="p-4 bg-white/5 rounded-2xl border border-white/10 space-y-2">
                  <h4 className="font-bold text-sm text-emerald-400">Instagram Stories</h4>
                ...
                <div className="p-4 bg-white/5 rounded-2xl border border-white/10 space-y-2">
                  <h4 className="font-bold text-sm text-emerald-400">Prova Social</h4>
            ...
            <div className="bg-emerald-50 p-8 rounded-3xl border border-emerald-100 space-y-4">
              <h2 className="text-xl font-bold text-emerald-900 flex items-center gap-2">
                <CheckCircle size={24} className="text-emerald-600" /> Suporte VIP
              </h2>
              <p className="text-sm text-emerald-700">Dúvidas sobre pagamentos ou produtos? Fale diretamente com nosso gerente de afiliados.</p>
              <a 
                href={`https://wa.me/${settings?.whatsapp_number}`}
                target="_blank"
                rel="noopener noreferrer"
                className="w-full bg-emerald-600 text-white py-3 rounded-xl font-bold text-center block hover:bg-emerald-700 transition-colors"
              >
                Chamar no WhatsApp
              </a>
            </div>
